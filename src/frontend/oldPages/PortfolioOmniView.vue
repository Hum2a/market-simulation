  ...
  <div class="portfolio-manage">
    ...
    <main class="main-content">
      ...
      <table class="portfolios-table">
        ...
        <tbody>
          <tr v-for="portfolio in filteredPortfolios" :key="portfolio.id" @click="viewPortfolio(portfolio.userId)">
  ...
  methods: {
    async fetchPortfolios() {
      ...
      for (const user of users) {
        console.log('Getting info for ', user.firstName);
        const portfolioCollection = collection(db, `${user.id}/Stock Trading Platform/Portfolio`);
        const portfolioQuery = query(portfolioCollection, where('__name__', '!=', 'Initial Portfolio'), orderBy('date', 'desc'), limit(1));
        ...
        portfolioSnapshot.forEach(doc => {
          ...
          this.portfolios.push({
            ...
            userId: user.id,
            totalValue: data.companies.reduce((sum, company) => sum + (company.currentValue || 0), 0),
            date: data.date,
    ...
    getChartData(companies) {
      ...
      return {
        ...
        datasets: [
          {
            label: 'Portfolio Value Of Each Stock',
    ...
    viewPortfolio(userId) {
      this.$router.push({ name: 'AdminPortfolioDisplayUID', params: { userId } });
    }
  }
};
</script>
