  ...
  export default {
    ...
    computed: {
      ...
      methods: {
        ...
        async fetchAdminStatus(user) {
          const db = getFirestore();
          const profileRef = doc(db, user.uid, 'Profile');
          const profileSnap = await getDoc(profileRef);
          if (profileSnap.exists()) {
            this.isAdmin = profileSnap.data().admin || false;
          }
        },
        ...
        async initializeData() {
          if (this.portfolio && this.portfolio.companies) {
            ...
            const updatedCompanies = await Promise.all(this.portfolio.companies.map(async (company, index) => {
              ...
              if (initialStockPrice === 0) {
                const realTimeData = await this.fetchRealTimeStockData(company.symbol);
            ...
            this.portfolio.companies = updatedCompanies;
          }
        },
...
.portfolio-display {
  ...
  align-items: center;
  background-color: #f0f2f5; /* Same as the body background color */
  min-height: 100vh;
  padding: 0;
  margin: 0;
  position: relative;
...
.refresh-button {
  background-color: #3498db; /* Matching color theme */
  color: #fff;
  padding: 0.5em 1.5em; /* Adjusted padding for better touch area */
  border: none;
...
.refresh-button:active {
  background-color: #1c598a; /* Even darker shade for active state */
...
.wallet-card {
  ...
  width: 100%;
  max-width: 150px; /* Ensure it doesn't exceed screen width */
  position: relative;
}
...
.button-link {
  ...
  margin-top: 10px;
  padding: 1px 2px; /* Add padding for better spacing */
  color: #fff;
...
.button-link:hover {
  background-color: #7f9cf5; /* Lighter shade for hover effect */
...
.financial-courses-card {
  ...
  transition: transform 0.3s, box-shadow 0.3s;
  width: 100%;
  max-width: 500px; /* Ensure it doesn't exceed screen width */
  box-sizing: border-box; /* Include padding in element's total width and height */
}
...
.portfolio-table {
  ...
  table-layout: fixed; /* Add this to make sure table doesn't exceed parent width */
}

.portfolio-table-card {
  overflow-x: auto; /* Add this to make the table scrollable on overflow */
}
...
.portfolio-table td {
  ...
  padding: 0.75em;
  text-align: left;
  font-size: 0.8em; /* Make font size smaller for the table */
}
...
.sticky-notes li {
  ...
  width: 100%;
  max-width: 300px; /* Ensure it doesn't exceed screen width */
...
@media (max-width: 768px) {
  ...
  .financial-courses-card {
    width: 100%;
    max-width: 100%; /* Ensure it takes the full width of the mobile view */
    box-sizing: border-box; /* Include padding in element's total width and height */
  }
  ...
  .portfolio-table td {
    ...
    font-size: 0.7em; /* Further reduce font size for mobile */
  }
